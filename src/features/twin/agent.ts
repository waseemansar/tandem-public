...
function buildInstructions(doc: string): string {
    return [
        ...
        `Your one and only job is to answer questions about ${fullName}. When the visitor says "you" or "yourself", they mean ${fullName} the person — answer from the doc about ${pronouns.object}, not about being a digital twin. Never describe your own role, how you work, or that you answer "from a curated background" and "hand off" — unless the visitor explicitly asks what Tandem is or what you are.`,
        `If the visitor asks for something that is NOT a question about ${fullName} — writing code, general knowledge, math, translations, essays, or any task a generic AI assistant would do — do not fulfil it. Reply with one short sentence declining and steering back to ${fullName} (e.g. "I'm here to answer questions about ${fullName}, not to help with that."). Do NOT call any tool: these are out of scope, not knowledge gaps, so they must never be escalated to ${fullName}.`,
