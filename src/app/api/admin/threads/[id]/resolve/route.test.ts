...
import { RESOLVED_SYSTEM_MESSAGE } from "@/features/conversation/constants";
...
import { POST } from "@/app/api/admin/threads/[id]/resolve/route";

const SESSION = { user: { email: "admin@example.com" } };
const RESOLVED_MESSAGE = RESOLVED_SYSTEM_MESSAGE;
...
function makeRequest(id: string): {
    ...
    return {
        request: new Request(`http://localhost/api/admin/threads/${id}/resolve`, {
            method: "POST",
        }),
...
describe("POST /api/admin/threads/[id]/resolve", () => {
    beforeAll(async () => {
        await startTestDb();
    });
    ...
    it.each(["twin_only", "awaiting_you", "active_you", "awaiting_visitor"] as const)(
        "from %s, transitions to resolved, persists system message, sets resolvedAt, broadcasts events",
        async (from) => {
            authMock.mockResolvedValue(SESSION);
            const id = await seedThread({ state: from, email: "x@y.io" });
            ...
            const messageEvents = events.filter(
                (ev): ev is SseMessageEvent => ev.type === "message",
            );
            expect(messageEvents).toHaveLength(1);
            expect(messageEvents[0].message.sender).toBe("system");
            expect(messageEvents[0].message.content).toBe(RESOLVED_MESSAGE);
            ...
            expect(rows[0].content).toBe(RESOLVED_MESSAGE);

            const [convo] = await getDb()
                .select({ state: conversations.state, resolvedAt: conversations.resolvedAt })
                .from(conversations)
                .where(eq(conversations.id, id));
            expect(convo.state).toBe("resolved");
            expect(convo.resolvedAt).not.toBeNull();
        },
    );
    ...
    it("returns 409 when the thread is already resolved", async () => {
        authMock.mockResolvedValue(SESSION);
        const id = await seedThread({ state: "resolved", email: "x@y.io" });
        const { request, ctx } = makeRequest(id);
        const res = await POST(request, ctx);
